...
class DailyBalanceRepository(BaseRepository[DailyBalance]):
    ...
    def get_by_subscription_and_date(
        self, subscription_id: uuid.UUID, balance_date
    ) -> Optional[DailyBalance]:
        return (
            self.db.query(DailyBalance)
            .filter(
    ...
    def get_by_subscription_id(
        ...
        query = self.db.query(DailyBalance).filter(
            DailyBalance.subscription_id == subscription_id
        )
        if restaurant_id is not None:
    ...
    def get_current_balance(
        ...
        if (
            ...
            and balance.restaurant_id != restaurant_id
        ):
            return None
        return balance

    def get_current_balance_for_update(
        self, subscription_id: uuid.UUID, restaurant_id: uuid.UUID | None = None
    ) -> Optional[DailyBalance]:
        query = self.db.query(DailyBalance).filter(
            DailyBalance.subscription_id == subscription_id,
