...
class TransactionRepository(BaseRepository[Transaction]):
    ...
    def get_by_pos_transaction_id(
        self, pos_transaction_id: str, restaurant_id: uuid.UUID | None = None
    ) -> Optional[Transaction]:
        query = self.db.query(Transaction).filter(
            Transaction.pos_transaction_id == pos_transaction_id
        )
        if restaurant_id is not None:
            query = query.filter(Transaction.restaurant_id == restaurant_id)
        return query.first()
    ...
    def get_by_idempotency_key(
        self, idempotency_key: str, restaurant_id: uuid.UUID | None = None
    ) -> Optional[Transaction]:
        query = self.db.query(Transaction).filter(
    ...
    def pos_transaction_id_exists(
        self, pos_transaction_id: str, restaurant_id: uuid.UUID | None = None
    ) -> bool:
        return (
            self.get_by_pos_transaction_id(pos_transaction_id, restaurant_id)
            is not None
        )
