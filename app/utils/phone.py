...
def normalize_cameroon_mobile(
    raw: str, operators: tuple[MobileOperator, ...] = MOBILE_OPERATORS
) -> tuple[str, MobileOperator]:
    ...
    for operator in operators:
        matching = [
            prefix for prefix in operator.prefixes if national.startswith(prefix)
        ]
        if matching:
