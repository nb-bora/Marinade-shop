...
class Settings(BaseSettings):
    ...
    ALLOW_HEADERS: str = "Authorization,Content-Type,X-Request-ID,X-Tenant-ID,X-Signature"
    ...
    def validate_security_configuration(self):
        if self.is_production:
            if self.SECRET_KEY == "your-secret-key-here-change-this-in-production" or len(self.SECRET_KEY) < 32:
                raise ValueError("SECRET_KEY must be a private random value of at least 32 characters in production")
            if self.DEBUG:
                raise ValueError("DEBUG must be false in production")
            if self.ALLOW_ORIGINS.strip() == "*" or not self.ALLOW_ORIGINS.strip():
                raise ValueError("ALLOW_ORIGINS must be an explicit allow-list in production")
            if not self.SKIP_DB_INIT:
                raise ValueError("SKIP_DB_INIT must remain true in production; use Alembic")
            if self.ALLOW_CREDENTIALS and self.ALLOW_METHODS.strip() == "*":
                raise ValueError("Wildcard methods cannot be used with credentialed CORS")
            ...
            if not self.EASYTRANSACT_WEBHOOK_SECRET:
                raise ValueError("Easy Transact webhook secret is required in production")
        return self
    ...
    def cors_methods(self) -> List[str]:
        return [method.strip().upper() for method in self.ALLOW_METHODS.split(",") if method.strip()]
