...
from app.models.restaurant import (
    Boisson, Combinaison, Commande, Composant, Menu, MenuCategory,
    Plat, Restaurant, Table,
)
...
def set_db_context(db: Session, name: str, value: str | None) -> None:
    ...
    db.execute(text("SELECT set_config(:name, :value, true)"), {"name": name, "value": value or ""})


def current_tenant_id(db: Session) -> uuid.UUID | None:
    value = db.execute(text("SELECT current_setting('app.current_tenant_id', true)")).scalar()
    if not value:
        return None
    try:
...
def _authorize_tenant(db: Session, user: User, tenant_id: uuid.UUID) -> Restaurant:
    if user.role == "admin":
        ...
        owned = db.query(Restaurant).filter(
...
def _set_requested_tenant(db: Session, user: User, request: Request) -> None:
    ...
    memberships = db.query(RestaurantMember).filter(
        RestaurantMember.user_id == user.id,
        RestaurantMember.is_active == True,
    ).all()
    if len(memberships) > 1:
        raise HTTPException(status_code=400, detail="X-Tenant-ID is required for a user with multiple restaurants")
    if memberships:
...
def get_current_user(
    ...
    set_db_context(db, "app.is_platform_admin", "true" if user.role == "admin" else "false")
    if user.role != "admin":
...
def require_pos(current_user: User = Depends(get_current_user)) -> User:
    if current_user.role not in {"pos", "restaurant", "admin"}:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="POS access required")
    return current_user
...
def require_tenant_path(
    ...
    if raw_tenant:
        try:
            ...
            raise HTTPException(status_code=400, detail="restaurant_id must be a UUID") from exc
    ...
    for param, model in resource_models.items():
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"{param} must be a UUID") from exc
