...
class PaymentService:
    ...
    def process_payment(
        ...
        if isinstance(method_type, (list, tuple, set)):
            methods = [
                str(item).strip().lower() for item in method_type if str(item).strip()
            ]
        else:
            methods = [
                part.strip().lower()
                for part in str(method_type).split(",")
                if part.strip()
            ]
        if len(methods) != 1:
            return {
                "success": False,
                "error": "Exactly one payment method is required",
                "status": "method_error",
            }
        method = methods[0]
        try:
            ...
            if not payment_method.validate_payment_details(
    ...
    def refund_payment(
        ...
        try:
            method = PaymentFactory.create_payment_method(
                method_type,
                {**self.payment_config.get(method_type, {}), **(method_config or {})},
            )
    ...
    def validate_payment_details(
        self, method_type: str, payment_details: Dict[str, Any]
    ) -> bool:
        try:
            method: PaymentMethod = PaymentFactory.create_payment_method(
                method_type, self.payment_config.get(method_type, {})
            )
