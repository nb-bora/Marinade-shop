...
class OperatorService:
    ...
    def list_prefixes(self, country_code: str = "+237", include_inactive: bool = False) -> list[MobileOperatorPrefix]:
        query = self.db.query(MobileOperatorPrefix).filter(MobileOperatorPrefix.country_code == country_code)
    ...
    def create_prefix(self, data: MobileOperatorPrefixCreate) -> MobileOperatorPrefix:
        existing = self.db.query(MobileOperatorPrefix).filter(MobileOperatorPrefix.country_code == data.country_code, MobileOperatorPrefix.national_prefix == data.national_prefix).first()
        if existing:
    ...
    def update_prefix(self, prefix_id: uuid.UUID, data: MobileOperatorPrefixUpdate) -> MobileOperatorPrefix | None:
    ...
    def active_operators(self, country_code: str = "+237") -> list[MobileOperator]:
        ...
        for row in self.list_prefixes(country_code):
            item = grouped.setdefault(row.operator_code, {"code": row.operator_code, "name": row.operator_name, "prefixes": []})
            item["prefixes"].append(row.national_prefix)  # type: ignore[union-attr]
        return [MobileOperator(code=str(item["code"]), name=str(item["name"]), prefixes=tuple(sorted(item["prefixes"], key=len, reverse=True))) for item in grouped.values()]  # type: ignore[arg-type]
