...
def upgrade() -> None:
    ...
    for table in ("subscriptions", "daily_balances", "transactions"):
        op.add_column(table, sa.Column("restaurant_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.execute("UPDATE subscriptions s SET restaurant_id=r.id FROM restaurants r WHERE r.user_id=s.user_id AND s.restaurant_id IS NULL")
    op.execute("UPDATE daily_balances b SET restaurant_id=s.restaurant_id FROM subscriptions s WHERE s.id=b.subscription_id AND b.restaurant_id IS NULL")
    op.execute("UPDATE transactions t SET restaurant_id=s.restaurant_id FROM subscriptions s WHERE s.id=t.subscription_id AND t.restaurant_id IS NULL")
    op.execute("DO $$ BEGIN IF EXISTS (SELECT 1 FROM subscriptions WHERE restaurant_id IS NULL) OR EXISTS (SELECT 1 FROM daily_balances WHERE restaurant_id IS NULL) OR EXISTS (SELECT 1 FROM transactions WHERE restaurant_id IS NULL) THEN RAISE EXCEPTION 'Tenant backfill failed'; END IF; END $$")
    for table in ("subscriptions", "daily_balances", "transactions"):
        op.alter_column(table, "restaurant_id", nullable=False)
    op.create_foreign_key("fk_subscription_restaurant", "subscriptions", "restaurants", ["restaurant_id"], ["id"], ondelete="RESTRICT")
    op.create_foreign_key("fk_daily_balance_restaurant", "daily_balances", "restaurants", ["restaurant_id"], ["id"], ondelete="RESTRICT")
    op.create_foreign_key("fk_transaction_restaurant", "transactions", "restaurants", ["restaurant_id"], ["id"], ondelete="RESTRICT")
    op.execute("DROP INDEX IF EXISTS idx_active_subscription_per_user")
    op.execute("ALTER TABLE transactions DROP CONSTRAINT IF EXISTS transactions_pos_transaction_id_key")
    op.execute("ALTER TABLE transactions DROP CONSTRAINT IF EXISTS transactions_idempotency_key_key")
    op.create_index("idx_active_subscription_per_restaurant", "subscriptions", ["restaurant_id"], unique=True, postgresql_where=sa.text("status = 'active'"))
    op.create_index("idx_transaction_pos_id_tenant", "transactions", ["restaurant_id", "pos_transaction_id"], unique=True)
    op.create_index("idx_transaction_idempotency_tenant", "transactions", ["restaurant_id", "idempotency_key"], unique=True)
    op.create_table(
        "restaurant_members",
        ...
        sa.CheckConstraint("role IN ('owner', 'manager', 'cashier', 'kitchen', 'staff')", name="check_restaurant_member_role"),
    )
    op.create_index("idx_restaurant_member_user", "restaurant_members", ["user_id"])
    op.execute("INSERT INTO restaurant_members (id, restaurant_id, user_id, role) SELECT gen_random_uuid(), id, user_id, 'owner' FROM restaurants")
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("country_code", "national_prefix", name="uq_mobile_operator_prefix"),
        sa.CheckConstraint("national_prefix ~ '^[0-9]{2,4}$'", name="check_mobile_prefix_digits"),
    )
    op.create_index("idx_mobile_operator_prefix_lookup", "mobile_operator_prefixes", ["country_code", "national_prefix", "is_active"])
    for code, name, prefixes in (("MTN_CM", "MTN Cameroun", ("650", "651", "652", "653", "654", "67")), ("ORANGE_CM", "Orange Cameroun", ("655", "656", "657", "658", "659", "69"))):
        for prefix in prefixes:
            op.execute(f"INSERT INTO mobile_operator_prefixes (id, operator_code, operator_name, national_prefix) VALUES (gen_random_uuid(), '{code}', '{name}', '{prefix}')")
    op.create_table(
        "payment_configurations",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("restaurant_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("provider", sa.String(30), nullable=False, server_default="easytransact"),
        sa.Column("service_code", sa.String(50), nullable=False, server_default="DEPOSIT"),
        sa.Column("webhook_url", sa.String(500)),
        sa.Column("success_url", sa.String(500)),
        sa.Column("failure_url", sa.String(500)),
        sa.Column("vendor_reference_prefix", sa.String(30), nullable=False, server_default="MRD"),
        sa.Column("provider_account_ref", sa.String(255)),
        sa.Column("credential_env_key", sa.String(255), nullable=False, server_default="EASYTRANSACT_API_TOKEN"),
        sa.Column("webhook_secret_env_key", sa.String(255), nullable=False, server_default="EASYTRANSACT_WEBHOOK_SECRET"),
        ...
        sa.ForeignKeyConstraint(["restaurant_id"], ["restaurants.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("restaurant_id", "provider", name="uq_restaurant_payment_provider"),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("restaurant_id", "idempotency_key", name="uq_payment_tenant_idempotency"),
        sa.CheckConstraint("amount_fcfa > 0", name="check_payment_amount_positive"),
        sa.CheckConstraint("status IN ('initiated', 'pending', 'processing', 'success', 'failed', 'timeout', 'reversed', 'refunded', 'expired', 'manual_review')", name="check_payment_status"),
    )
    op.create_index("idx_payment_intent_restaurant_status", "payment_intents", ["restaurant_id", "status"])
    op.create_index("idx_payment_intent_commande", "payment_intents", ["commande_id"])
    op.add_column("commandes", sa.Column("payment_intent_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key("fk_commande_payment_intent", "commandes", "payment_intents", ["payment_intent_id"], ["id"], ondelete="SET NULL")
    op.create_unique_constraint("uq_commande_payment_intent", "commandes", ["payment_intent_id"])
    op.create_check_constraint("check_commande_status", "commandes", "statut IN ('en_cours', 'servie', 'annulee', 'payee', 'paiement_en_attente', 'paiement_a_verifier')")
    op.create_table(
        "payment_events",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
    ...
    op.create_index("idx_payment_event_intent", "payment_events", ["payment_intent_id", "received_at"])
    op.create_table(
        "payment_ledger",
        sa.Column("id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("payment_intent_id", postgresql.UUID(as_uuid=True), nullable=False),
        ...
        sa.Column("metadata_jsonb", postgresql.JSONB()),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.ForeignKeyConstraint(["payment_intent_id"], ["payment_intents.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["restaurant_id"], ["restaurants.id"], ondelete="RESTRICT"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("idempotency_key", name="uq_payment_ledger_idempotency"),
        sa.CheckConstraint("entry_type IN ('authorization', 'capture', 'reversal', 'refund', 'fee', 'manual_adjustment')", name="check_payment_ledger_entry_type"),
        sa.CheckConstraint("amount_fcfa <> 0", name="check_payment_ledger_amount_nonzero"),
    )
    op.create_index("idx_payment_ledger_restaurant_created", "payment_ledger", ["restaurant_id", "created_at"])
    op.execute("CREATE UNIQUE INDEX IF NOT EXISTS uq_restaurant_owner ON restaurants (user_id)")
    _context_functions()
    direct = {
    ...
    _rls("mobile_operator_prefixes", "(app_is_platform_admin() OR app_current_user_id() IS NOT NULL)")


def downgrade() -> None:
    for table in ("restaurants", "restaurant_members", "menus", "menu_categories", "composants", "combinaisons", "stock_composants", "plats", "boissons", "tables", "commandes", "subscriptions", "daily_balances", "transactions", "payment_configurations", "payment_intents", "payment_events", "payment_ledger", "mobile_operator_prefixes"):
    ...
    op.drop_constraint("fk_daily_balance_restaurant", "daily_balances", type_="foreignkey")
    op.drop_constraint("fk_subscription_restaurant", "subscriptions", type_="foreignkey")
