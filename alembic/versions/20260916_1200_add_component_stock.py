...
def upgrade():
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.CheckConstraint(
            "quantite > 0", name="check_stock_mouvement_quantite_positive"
        ),
    )
    op.create_index(
        "idx_stock_mouvement_composant", "stock_mouvements", ["composant_id"]
    )
...
def downgrade():
    ...
    op.drop_constraint(
        "uq_combinaison_composant", "combinaison_composants", type_="unique"
    )
    op.drop_constraint(
        "check_combinaison_composant_quantite_positive",
        "combinaison_composants",
