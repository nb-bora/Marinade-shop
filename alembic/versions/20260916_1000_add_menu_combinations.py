...
def upgrade():
    op.create_table(
        ...
        sa.Column("prix_supplement", sa.Numeric(precision=10, scale=2), nullable=False, server_default="0"),
        ...
        sa.CheckConstraint("prix_supplement >= 0", name="check_composant_supplement_positive"),
    )
    ...
    op.create_index("idx_composant_disponible", "composants", ["restaurant_id", "disponible"])
    ...
    op.create_index("idx_combinaison_disponible", "combinaisons", ["restaurant_id", "disponible"])
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()"), nullable=False),
        sa.ForeignKeyConstraint(["combinaison_id"], ["combinaisons.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["composant_id"], ["composants.id"], ondelete="CASCADE"),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index("idx_combinaison_composant_combinaison", "combinaison_composants", ["combinaison_id"])
    op.create_index("idx_combinaison_composant_composant", "combinaison_composants", ["composant_id"])

    op.add_column("commande_items", sa.Column("combinaison_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column("commande_items", sa.Column("supplements_total", sa.Numeric(precision=10, scale=2), nullable=False, server_default="0"))
    op.add_column("commande_items", sa.Column("details_jsonb", postgresql.JSONB(), nullable=True))
    op.create_foreign_key("fk_commande_item_combinaison", "commande_items", "combinaisons", ["combinaison_id"], ["id"], ondelete="SET NULL")
    op.create_index("idx_item_combinaison", "commande_items", ["combinaison_id"])
    op.create_check_constraint("check_item_supplements_positive", "commande_items", "supplements_total >= 0")


def downgrade():
    op.drop_constraint("check_item_supplements_positive", "commande_items", type_="check")
    op.drop_index("idx_item_combinaison", table_name="commande_items")
    op.drop_constraint("fk_commande_item_combinaison", "commande_items", type_="foreignkey")
