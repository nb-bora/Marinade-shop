...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Index(
            "idx_active_subscription_per_user",
            "user_id",
            unique=True,
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "initial_balance_fcfa > 0", name="check_initial_balance_positive"
        ),
        sa.CheckConstraint(
            "used_balance_fcfa >= 0", name="check_used_balance_non_negative"
        ),
        sa.CheckConstraint(
            "used_balance_fcfa <= initial_balance_fcfa",
            name="check_used_balance_within_limit",
        ),
        sa.UniqueConstraint(
            "subscription_id", "balance_date", name="uq_subscription_balance"
        ),
    )
