...
def upgrade():
    ...
    op.execute(
        "ALTER TABLE users ADD CONSTRAINT check_role_valid CHECK (role IN ('admin', 'pos', 'restaurant'))"
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "currency", sa.String(length=3), nullable=False, server_default="XAF"
        ),
    ...
    op.create_index(
        "idx_category_order", "menu_categories", ["restaurant_id", "menu_id", "ordre"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["category_id"], ["menu_categories.id"], ondelete="SET NULL"
        ),
        sa.PrimaryKeyConstraint("id"),
    ...
    op.create_table(
        ...
        sa.Column(
            "statut", sa.String(length=20), nullable=False, server_default="libre"
        ),
        sa.Column("attributs_jsonb", postgresql.JSONB(), nullable=True),
        sa.Column(
            "created_at",
    ...
    op.create_table(
        ...
        sa.Column(
            "statut", sa.String(length=20), nullable=False, server_default="en_cours"
        ),
...
def downgrade():
    ...
    op.execute("ALTER TABLE users DROP CONSTRAINT IF EXISTS check_role_valid")
    op.execute(
        "ALTER TABLE users ADD CONSTRAINT check_role_valid CHECK (role IN ('admin', 'pos'))"
    )
